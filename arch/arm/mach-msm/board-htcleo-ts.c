...
static void leo_ts_early_suspend(struct early_suspend *h);
static void leo_ts_late_resume(struct early_suspend *h);
#endif



static int I2C_Read(struct leo_ts_data *ts, uint8_t dev, uint8_t addr, uint32_t sz, uint8_t* bf)
{
...
static int I2C_ReadNo(struct leo_ts_data *ts, uint8_t dev, uint32_t sz, uint8_t* bf)
{
...
static int leo_detect_ts_type(struct leo_ts_data *ts)
{
    uint8_t bt[4];
...
static int leo_init_ts(struct leo_ts_data *ts)
{
    uint8_t bt[6];
    struct i2c_msg msg;
    int ret;
...
static void leo_deinit_ts(void)
{
     gpio_set_value(HTCLEO_GPIO_TS_POWER, 0);
}

static int leo_init_intr(struct leo_ts_data *ts)
{
    switch (ts->ts_type)
    {
    case TOUCH_TYPE_B8:
...
static void leo_ts_work_func(struct work_struct *work)
{
    uint8_t buf[9];
    uint32_t ptcount;
    uint32_t ptx[2];
...
static irqreturn_t leo_ts_irq_handler(int irq, void *dev_id)
{
    struct leo_ts_data *ts = dev_id;
    disable_irq_nosync(ts->client->irq);
    queue_work(leo_touch_wq, &ts->work);
    return IRQ_HANDLED;
}
...
static int leo_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    struct leo_ts_data *ts;
    int ret = 0;
    ...
    ts = kzalloc(sizeof(struct leo_ts_data), GFP_KERNEL);
    if (ts == NULL)
    {
        dev_err(&client->dev, "allocate leo_ts_data failed\n");
        ret = -ENOMEM;
        goto err_alloc_data_failed;
    }

    INIT_WORK(&ts->work, leo_ts_work_func);
    ts->client = client;
    ...
    gpio_request(82, "htcleo_ts");
    gpio_request(27, "htcleo_ts");


    ret = leo_reset_ts(ts);
    if (ret < 0)
    {
        dev_err(&client->dev, "TS: leo_reset_ts() failed\n");
        goto err_detect_failed;
    }

    ret = leo_detect_ts_type(ts);
    if (ret < 0)
    {
        dev_err(&client->dev, "TS: leo_detect_ts_type() failed\n");
        goto err_detect_failed;
    }
...
// !!!other types are not supported yet!!!
    ...
    if (ret < 0)
    {
        dev_err(&client->dev, "TS: leo_init_ts() failed\n");
        goto err_detect_failed;
    }

    ret = leo_init_intr(ts);
    if (ret < 0)
    {
        dev_err(&client->dev, "TS: leo_init_intr() failed\n");
        goto err_detect_failed;
    }
    ...
    ts->input_dev->name = "leo-touchscreen";
...
//    input_set_abs_params(ts->input_dev, ABS_TOOL_WIDTH, pdata->abs_width_min, pdata->abs_width_max, 0, 0);
    ...
    {
        dev_err(&client->dev,
            "leo_ts_probe: Unable to register %s input device\n",
            ts->input_dev->name);
        goto err_input_register_device_failed;
    }

    ts->client->irq = gpio_to_irq(HTCLEO_GPIO_TS_IRQ);
    ret = request_irq(ts->client->irq, leo_ts_irq_handler,
                  ts->intr_type ? IRQF_TRIGGER_HIGH : IRQF_TRIGGER_LOW,
                    LEO_TOUCH_DRV_NAME, ts);

#ifdef CONFIG_HAS_EARLYSUSPEND
    ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
    ts->early_suspend.suspend = leo_ts_early_suspend;
    ts->early_suspend.resume = leo_ts_late_resume;
...
static int leo_ts_remove(struct i2c_client *client)
{
    struct leo_ts_data *ts = i2c_get_clientdata(client);
...
static int leo_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
    struct leo_ts_data *ts = i2c_get_clientdata(client);
    int ret;
...
static int leo_ts_resume(struct i2c_client *client)
{
    struct leo_ts_data *ts = i2c_get_clientdata(client);

    // POWER ON
    leo_reset_ts(ts);
    leo_detect_ts_type(ts);
    leo_init_ts(ts);
    leo_init_intr(ts);
...
{
    struct leo_ts_data *ts;
    ts = container_of(h, struct leo_ts_data, early_suspend);
    leo_ts_suspend(ts->client, PMSG_SUSPEND);
}
...
static void leo_ts_late_resume(struct early_suspend *h)
{
    struct leo_ts_data *ts;
    ts = container_of(h, struct leo_ts_data, early_suspend);
    leo_ts_resume(ts->client);
}
#endif


static const struct i2c_device_id leo_ts_id[] =
{
...
static struct i2c_driver leo_ts_driver =
{
    .driver =
    {
    ...
    .id_table   = leo_ts_id,
    .probe = leo_ts_probe,
    .remove = leo_ts_remove,
#ifndef CONFIG_HAS_EARLYSUSPEND
    .suspend = leo_ts_suspend,
    .resume = leo_ts_resume,
#endif
};


static int __devinit leo_ts_init(void)
{
    leo_touch_wq = create_singlethread_workqueue("leo_touch_wq");
    if (!leo_touch_wq)
    {
        return -ENOMEM;
    }
    return i2c_add_driver(&leo_ts_driver);
}

static void __exit leo_ts_exit(void)
{
    if (leo_touch_wq)
    {
        destroy_workqueue(leo_touch_wq);
        leo_touch_wq = NULL;
    }
    i2c_del_driver(&leo_ts_driver);
}

module_init(leo_ts_init);
module_exit(leo_ts_exit);
